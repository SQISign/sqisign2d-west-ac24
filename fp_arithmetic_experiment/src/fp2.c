...
//     memcpy((uint64_t*)exp_tmp, (uint64_t*)exp, size*RADIX/8);
//     memcpy((uint64_t*)acc.re, (uint64_t*)&(x->re), NWORDS_FIELD*RADIX/8);
//     memcpy((uint64_t*)acc.im, (uint64_t*)&(x->im), NWORDS_FIELD*RADIX/8);
//     fp_set(out->re, 1);
