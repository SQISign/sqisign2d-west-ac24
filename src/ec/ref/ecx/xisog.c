...
void
xisog_2(ec_point_t *B, ec_point_t const P)
{
    fp2_sqr(&B->x, &P.x);
    fp2_sqr(&B->z, &P.z);
    fp2_sub(&B->x, &B->z, &B->x);
...
// // Degree-2 isogeny when the kernel is P = (0,0)
// // uses a point Q of order 8 above P
// void xisog_2_singular_new(ec_point_t *A24,const ec_point_t *Q) {

// 	#ifndef NDEBUG
// 	fp2_t testQ,test24;
// 	fp2_copy(&testQ,&Q->z);
// 	fp2_inv(&testQ);
// 	fp2_mul(&testQ,&testQ,&Q->x);
// 	fp2_sqr(&testQ,&testQ);

// 	fp2_copy(&test24,&A24->z);
// 	fp2_inv(&test24);
// 	fp2_mul(&test24,&test24,&A24->x);

// 	// now we must have 16 test_24 = testQ
// 	fp2_add(&test24,&test24,&test24); // 2 test24
// 	fp2_add(&test24,&test24,&test24); // 4 test24
// 	fp2_add(&test24,&test24,&test24); // 8 test24
// 	fp2_add(&test24,&test24,&test24); // 16 test24

// 	assert(fp2_is_equal(&test24,&testQ));
// 	printf("yaya !! \n");
// 	#endif
...
void
xisog_2_singular(ec_point_t *B24, ec_point_t A24)
{
    fp2_t t0, four;
...
void
xeval_2_singular(ec_point_t *R, const ec_point_t *Q, const int lenQ)
{
    fp2_t t0, t1;
...
// Outputs the curve coefficient in the form A24=(A+2C:4C)
void
xisog_4(ec_point_t *B, ec_point_t const P)
{
    fp2_sqr(&K[0].x, &P.x);
    fp2_sqr(&K[0].z, &P.z);
...
// Degree-4 isogeny with kernel generated by P such that [2]P = (0 ,0)
void
xisog_4_singular(ec_point_t *B24, ec_point_t const P, ec_point_t A24)
{
...
void
xisog_s(ec_point_t *B, uint64_t const i, ec_point_t const A)
{
    // =================================================================================
    ...
    //                   ~~~~~~~~
    //                    |    |
    // Computing E_J(W) = |    | [ F0(W, x([j]P)) * alpha^2 + F1(W, x([j]P)) * alpha + F2(W,
    // x([j]P)) ]
    //                    j in J
    // In order to avoid costly inverse computations in fp, we are gonna work with projective
    // coordinates In particular, for a degree-l isogeny construction, we need alpha = 1 and alpha =
    // -1
    ...
    fp2_t A24, A24m, t24,
        t24m; // <---- JORGE creo que podemos omitir estas variables, se usan cuando ya no se
              // requiren los valores de la entrada A (podemos cambiar estos t's por B[0] y B[1]
