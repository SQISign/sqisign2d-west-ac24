...
int main(int argc, char* argv[])
{
	...
	for(int iter = 0; iter < TEST_LOOPS/10; iter++){
		...
		fp2_from_w64(&P.x,  xPA_data);
		fp2_from_w64(&Q.x,  xQA_data);
		fp2_from_w64(&PQ.x, xPQA_data);
		fp2_set_one(&P.z);
		fp2_set_one(&Q.z);
		fp2_set_one(&PQ.z);
		...
		fp2_from_w64(&P.x,  xPB_data);
		fp2_from_w64(&Q.x,  xQB_data);
		fp2_from_w64(&PQ.x, xPQB_data);
		fp2_set_one(&P.z);
		fp2_set_one(&Q.z);
		fp2_set_one(&PQ.z);
