...
fp2_random(fp2_t *a)
{
    ...
    fp_encode(&tmp, &(a->re));
    unsigned seed =
        (unsigned)tmp[0] | (unsigned)tmp[1] << 8 | (unsigned)tmp[2] << 16 | (unsigned)tmp[3] << 24;
    srand((unsigned)seed);
}
...
main()
{
    ...
    for (tree_size = 3; tree_size < nmax; tree_size++) {
        printf("[%3d%%] Testing reciprocal tree:\t\tTree size %d out of %d",
               100 * tree_size / nmax,
               tree_size,
               nmax);
        fflush(stdout);
    ...
    for (tree_size = 2; tree_size < nmax; tree_size++) {
        printf("[%3d%%] Testing batched remainders:\t\tTree size %d out of %d",
               100 * tree_size / nmax,
               tree_size,
               nmax);
        fflush(stdout);
    ...
    for (tree_size = 1; tree_size < nmax; tree_size++) {
        printf("[%3d%%] Testing scaled remainder tree:\tTree size %d out of %d",
               100 * tree_size / nmax,
               tree_size,
               nmax);
        fflush(stdout);
