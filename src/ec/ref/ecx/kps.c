...
int deg_ptree_hI[(1 << (ceil_log_sI_max + 2)) -
...
fp2_t
    leaves[sI_max]; // leaves of the remainder tree, which are required in the Resultant computation
...
void
kps_s(uint64_t const i, ec_point_t const P, ec_point_t const A)
{
    // =================================================================================
    ...
    //                    i in I                 i in I
    // In order to avoid costly inverse computations in fp, we are gonna work with projective
    // coordinates
    ...
    if (!scaled) {
        ...
        reciprocal_tree(
            rtree_hI, rtree_A, 2 * sJ + 1, ptree_hI, deg_ptree_hI, 0, sI); // Reciprocal tree of hI
    } else {
