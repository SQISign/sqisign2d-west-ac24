...
void
xeval_2(ec_point_t *R, ec_point_t *const Q, const int lenQ)
{
    fp2_t t0, t1, t2;
...
void
xeval_4(ec_point_t *R, const ec_point_t *Q, const int lenQ)
{
    fp2_t t0, t1;
...
void
xeval_4_singular(ec_point_t *R, const ec_point_t *Q, const int lenQ, const ec_point_t P)
{
    fp2_t t0, t1, t2;
    for (int i = 0; i < lenQ; i++) {
        fp2_add(&t0, &Q[i].x, &Q[i].z);
...
void
xeval_s(ec_point_t *Q, uint64_t const i, ec_point_t const P, ec_point_t const A)
{
    // =================================================================================
    ...
    // coordinates In particular, for a degree-l isogeny construction, we need alpha = X/Z and alpha
    // = Z/X (i.e., 1/alpha)
    ...
    for (j = 0; j < sJ; j++) {
        ...
        // C * [ (-2*Xj*Zj)*(alpha^2 + 1) + (-2*alpha)*(Xj^2 + Zj^2)] + [A' * (-2*Xj*Zj) * (2*X*Z)]
        // where alpha = X/Z
        ...
        fp2_sub(
            &t1,
            &t2,
            &t1); // [C * (X^2 + Z^2)] * (-4 * Xj * Zj) - [2 * (Xj^2 + Zj^2)] * (2 * [ C * (X * Z)])
    ...
    fp2_mul(
        &t1, &hk_1, &r1); // output of algorithm 2 with 1/alpha = Z/X and without the demoninator
    fp2_sqr(&t1, &t1);
    fp2_mul(&(Q->x), &t1, &P.x);
