...
{ // Tests for the GF(p^2) arithmetic
    ...
    {
        fp2_random_test(&a); fp2_random_test(&b); fp2_random_test(&c); fp2_random_test(&d); 

        fp2_add(&d, &a, &b); fp2_add(&e, &d, &c);                 // e = (a+b)+c
        fp2_add(&d, &b, &c); fp2_add(&f, &d, &a);                 // f = a+(b+c)
        if (fp2_is_equal(&e, &f) == 0) { passed=0; break; }

        fp2_add(&d, &a, &b);                                      // d = a+b 
        fp2_add(&e, &b, &a);                                      // e = b+a
        if (fp2_is_equal(&d, &e) == 0) { passed=0; break; }

        fp2_set_zero(&b);
        fp2_add(&d, &a, &b);                                      // d = a+0 
        if (fp2_is_equal(&d, &a) == 0) { passed=0; break; }

        fp2_neg(&d, &a);                      
        fp2_add(&e, &a, &d);                                      // e = a+(-a)
        if (fp2_is_zero(&e) == 0) { passed=0; break; }
    }
    ...
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fp2_random_test(&a); fp2_random_test(&b); fp2_random_test(&c);
        
        fp2_mul(&d, &a, &b); fp2_mul(&e, &d, &c);                          // e = (a*b)*c
        fp2_mul(&d, &b, &c); fp2_mul(&f, &d, &a);                          // f = a*(b*c)
        if (fp2_is_equal(&e, &f) == 0) { passed=0; break; }

        fp2_add(&d, &b, &c); fp2_mul(&e, &a, &d);                          // e = a*(b+c)
        fp2_mul(&d, &a, &b); fp2_mul(&f, &a, &c); fp2_add(&f, &d, &f);     // f = a*b+a*c
        if (fp2_is_equal(&e, &f) == 0) { passed=0; break; }
        ...
        fp2_set_one(&b);
        fp2_mul(&d, &a, &b);                                                  // d = a*1  
        if (fp2_is_equal(&a, &d) == 0) { passed=0; break; }
       
        fp2_set_zero(&b);
        fp2_mul(&d, &a, &b);                                                  // d = a*0 
        if (fp2_is_zero(&d) == 0) { passed=0; break; }
    }
    ...
    for (n=0; n<TEST_LOOPS; n++)
    {
        ...
        fp2_sqr(&b, &a);                                          // b = a^2
        fp2_mul(&c, &a, &a);                                      // c = a*a 
        if (fp2_is_equal(&b, &c) == 0) { passed=0; break; }
    ...
    for (n=0; n<TEST_LOOPS; n++)
    {
        ...
        if (fp2_is_equal(&c, &d) == 0) { passed=0; break; }
    ...
    for (n=0; n<TEST_LOOPS; n++)
    {
        ...
        fp2_sqr(&c, &a);                                  // c = a^2
        if (fp2_is_square(&c) != 1) { passed = 0; break; }        

        fp2_sqrt(&c);                                              // c = a = sqrt(c) 
        fp2_neg(&d, &c);
        if ((fp2_is_equal(&a, &c) == 0) && (fp2_is_equal(&a, &d) == 0)) { passed = 0; break; }
    }
